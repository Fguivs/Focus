  ...
  function atualizarExibicaoArvore() {
   ...
   async function verificarMudancaData() {
    ...
    if (dataAtual !== novaData) {
      // O dia mudou, vamos executar toda a rotina de um novo dia.
      dataAtual = novaData;
      
      // 1. Tarefas administrativas de início de dia
      await limparColecaoSemanas(); 
      await limparMuralSemanal();
      await carregarAniversariantes();

      // 2. Lógica de finalização/reset
      const agora = new Date();
      const horasBrasilia = agora.getBrasiliaHours();
      const hoje = getHoje();

      // 2a. Resetar o dia se for meia-noite
      if (horasBrasilia === 0 && agora.getMinutes() === 0) {
        await resetarDia(true);
      }
      
      // 2b. Finalizar a competição da semana se for domingo
      if (hoje.getDay() === 0 && horasBrasilia >= 0) {
        await finalizarSemana();
      }

      // 3. ATUALIZAÇÃO COMPLETA DA INTERFACE (A MÁGICA ACONTECE AQUI)
      // Recarrega todos os dados visuais importantes para o novo dia.
      
      atualizarDataCabecalho();    // Atualiza a data no cabeçalho.
      atualizarInfoSemana();      // Atualiza o número e as datas da semana.
      await carregarPontosSemanais(); // Carrega (ou zera) os pontos da semana.
      atualizarPlacarSemanal();     // Exibe os pontos semanais na tela.
      await carregarArvoreEpica();   // Verifica o estado da árvore épica.
      await atualizarResumo();      // ATUALIZA O CONTADOR GERAL, BARRA DE PROGRESSO e outros resumos.

      console.log("Novo dia detectado! Interface atualizada automaticamente.");

    } else {
        // Se o dia não mudou, apenas atualize o cabeçalho de data caso ele se perca.
...
async function openMessageViewer(messageId) {
  // Se já houver um ouvinte ativo de um modal anterior, remove-o
  ...
  // Adiciona um ID ao modal para sabermos qual mensagem está aberta
  viewerModal.dataset.activeMessageId = messageId; 

  const mensagemRef = doc(db, "mural", messageId);

  // Cria um ouvinte EM TEMPO REAL para esta mensagem específica
  unsubscribeViewerListener = onSnapshot(mensagemRef, (docSnap) => {
    if (!docSnap.exists()) {
      // Se a mensagem for deletada enquanto estiver aberta, fecha o modal
    ...
    // Preenche os dados básicos (só precisa fazer na primeira vez, mas não prejudica)
    ...
    // --- LÓGICA DE ATUALIZAÇÃO DAS REAÇÕES (Será executada sempre que houver uma mudança) ---
    const reacoes = mensagem.reacoes || {};
    let reacoesHTML = '';
    let temReacoes = false;

    for (const emoji in reacoes) {
      if (Object.prototype.hasOwnProperty.call(reacoes, emoji) && reacoes[emoji] > 0) {
        temReacoes = true;
        reacoesHTML += `
          <div class="reacao-display">
            ${emoji} <span class="contador-display">${reacoes[emoji]}</span>
          </div>
        `;
      }
    }
    ...
    // Configura a barra para adicionar novas reações
    const viewerSeletorBar = viewerModal.querySelector('.reacao-seletor-bar');

// 1. Limpa a barra de reações para evitar botões duplicados
viewerSeletorBar.innerHTML = '';

// 2. Cria e adiciona os botões de reação novamente
const emojis = ['👍', '😂', '😭', '💖', '😡'];
emojis.forEach(emoji => {
    ...
    btn.onclick = (e) => {
        e.stopPropagation();
        window.adicionarReacao(messageId, emoji);
    };
    viewerSeletorBar.appendChild(btn);
});

// 3. Garante que a barra de reações fique sempre visível
...
window.enviarMensagem = async function() {
  ...
  try {
    if (composerEditMode && editingMessageId) { // Atualiza mensagem existente
      ...
      dadosMensagem.reacoes = { "👍": 0, "😂": 0, "😭": 0, "💖": 0, "😡": 0 };
...
function criarElementoMensagem(mensagem) {
  ...
  mensagemEl.innerHTML = `
    ...
    <div class="reacao-seletor-bar hidden">
      <button onclick="event.stopPropagation(); window.adicionarReacao('${mensagem.id}', '👍')">👍</button>
      <button onclick="event.stopPropagation(); window.adicionarReacao('${mensagem.id}', '😂')">😂</button>
      <button onclick="event.stopPropagation(); window.adicionarReacao('${mensagem.id}', '😭')">😭</button>
      <button onclick="event.stopPropagation(); window.adicionarReacao('${mensagem.id}', '💖')">💖</button>
      <button onclick="event.stopPropagation(); window.adicionarReacao('${mensagem.id}', '😡')">😡</button>
  ...
  const startPress = (e) => {
    // Ignora o clique longo se for em um botão já existente
    ...
    pressTimer = setTimeout(() => {
      ...
      if (seletorBar) {
        ...
        // Define a função que decide quando fechar a barra
        const closeReactionBar = (event) => {
          // Verifica se o clique foi em um botão de emoji
          const isReactionButton = event.target.closest('.reacao-seletor-bar button');

          // Fecha a barra se o clique for FORA do card OU em um botão de reação
          if (!mensagemEl.contains(event.target) || isReactionButton) {
            seletorBar.classList.add('hidden');
            // Remove o ouvinte de cliques do documento para não ficar ativo para sempre
            document.removeEventListener('click', closeReactionBar);
          }
        };

        // Adiciona o ouvinte ao documento para capturar o próximo clique
        // O timeout evita que o próprio clique do "long press" feche a barra
        setTimeout(() => {
          document.addEventListener('click', closeReactionBar);
        }, 100);
      }
    }, 500); // 500ms para o clique longo
  };
  ...
  const endPress = (e) => {
    ...
    // Se não foi um clique longo, abre o visualizador da mensagem
  ...
  mensagemEl.addEventListener('touchmove', cancelPress);
  ...
  if (optionsBtn) {
    optionsBtn.addEventListener('click', (e) => {
      ...
      // Se o menu estava escondido, ele será exibido
      if (isHidden) {
        dropdown.classList.remove('hidden');

        // Adiciona um ouvinte global para fechar o menu ao clicar fora
        const closeOnClickOutside = (event) => {
          // Se o clique NÃO foi dentro do menu
          if (!dropdown.contains(event.target) && !optionsBtn.contains(event.target)) {
            dropdown.classList.add('hidden');
            // Remove o ouvinte para não ficar rodando desnecessariamente
            document.removeEventListener('click', closeOnClickOutside);
          }
        };
        
        // Registra o ouvinte no documento inteiro
  ...
  window.adicionarReacao = async function(mensagemId, emoji) {
    try {
      ...
      if (docSnap.exists()) {
        ...
        // Incrementar contador para esta reação
...
async function loadAdvantageState() {
    ...
    // ===== INÍCIO DA MODIFICAÇÃO PARA O LÍDER =====
    const leaderControls = document.getElementById('leader-advantage-controls');
    if (userRole === 'lider') {
        // Se o usuário for o líder geral, mostra o botão de teste
        leaderControls.classList.remove('hidden');
        document.getElementById('leader-test-button').onclick = window.toggleLeaderTestPanel;
        populateLeaderTestPanel(); // Popula a lista de jogos

        // O líder pode testar, então o resto da função não o bloqueará
        // Mas ainda deixamos o jogo padrão da semana carregar para ele ver
    } else {
        // Garante que para outros usuários o painel esteja sempre escondido
        leaderControls.classList.add('hidden');
    }
    // ===== FIM DA MODIFICAÇÃO PARA O LÍDER =====

    const hoje = getHoje();
    const diaSemana = hoje.getDay();

    if (diaSemana === 0 || diaSemana === 6) {
        // O líder também vê a mensagem de fim de semana, mas pode testar mesmo assim.
        lockAdvantageSection("O Jogo da Vantagem retorna na Segunda-feira. Bom descanso!");
        if(userRole !== 'lider') {
    ...
    if (docSnap.exists() && docSnap.data().completadoPor?.[currentUser]) {
...
async function saveAdvantageCompletion() {
    ...
    const advantageRef = doc(db, "vantagemSemanal", docId);
    try {
        await setDoc(advantageRef, { completadoPor: { [currentUser]: true } }, { merge: true });
